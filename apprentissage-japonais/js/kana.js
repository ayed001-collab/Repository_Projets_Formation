/**
 * kana.js — Table des kana (hiragana & katakana)
 * ----------------------------------------------
 * Les deux syllabaires japonais. Chaque case : { h: hiragana, k: katakana,
 * r: rōmaji } ; null = case vide (pour garder l'alignement du tableau).
 *
 * Trois tableaux :
 *   gojuon  — les 46 sons de base (5 colonnes : a i u e o)
 *   dakuten — sons voisés ゛ et ゜ (が, ざ, だ, ば, ぱ…)
 *   yoon    — combinaisons contractées (きゃ, しゃ…)
 *
 * Ce fichier expose la variable globale KANA.
 */
const KANA = {
  gojuon: {
    titre: "Gojūon — les 46 sons de base",
    cols: 5,
    rows: [
  ...
  dakuten: {
    titre: "Dakuten & handakuten — sons voisés (゛ ゜)",
    cols: 5,
    rows: [
  ...
  yoon: {
    titre: "Yōon — combinaisons contractées",
    cols: 3,
    rows: [
...
/* Exposé global */
if (typeof window !== "undefined") window.KANA = KANA;
